/**
 * EquazioniSecondoGradoDemo - Versione Responsive (stile "DisequazioniSecondoGradoDemo")
 * Step-by-step: forma normale → classificazione → risoluzione → risultato
...
export default function EquazioniSecondoGradoDemo() {
    ...
    const numericWorkLatex = useMemo(() => {
        ...
        if (eq.eqType === "pura") {
            ...
            if (rhs < 0)
                return `\\begin{aligned} ${line1} \\\\ ${line2} \\\\ ${line3} \\\\ &\\Rightarrow \\text{nessuna soluzione reale} \\end{aligned}`;
            if (isZero(rhs))
                return `\\begin{aligned} ${line1} \\\\ ${line2} \\\\ ${line3} \\\\ &\\Rightarrow x = 0 \\end{aligned}`;
            return `\\begin{aligned} ${line1} \\\\ ${line2} \\\\ ${line3} \\\\ &\\Rightarrow x = \\pm \\sqrt{${formatNumberLatex(
                rhs
            )}} \\\\ &\\Rightarrow ${eq.solutionsLatex} \\end{aligned}`;
        }
        ...
        if (delta < 0)
            return `\\begin{aligned} ${line0} \\\\ ${line1} \\\\ ${line2} \\\\ &\\Rightarrow \\Delta < 0 \\Rightarrow \\text{nessuna soluzione reale} \\end{aligned}`;
        ...
        const line5 = `\\Rightarrow ${eq.solutionsLatex}`;
    ...
    const Step2 = (
        <StepCard stepNumber={2} title="Classificazione" color="blue" isActive={isActive(2)}>
            ...
            {eq.eqType === "completa" && (
                <div style={{ marginTop: 10, fontSize: 13, color: "#64748b" }}>
                    <Latex>{`\\Delta = ${eq.b}^2 - 4\\cdot ${eq.a} \\cdot ${eq.c} = ${eq.delta}`}</Latex>
                </div>
            )}
        </StepCard>
    );
        ...
        <StepCard stepNumber={3} title="Risoluzione" color="amber" isActive={isActive(3)} fullWidth>
            ...
            >
                <div style={{ background: "#fff", borderRadius: 8, padding: 12 }}>
                    ...
                    <div style={{ fontSize: 16, lineHeight: 1.6, overflowX: "auto" }}>
    ...
    const Step4 = (
        <StepCard
            stepNumber={4}
            title="Risultato"
            color={eq.hasRealSolutions ? "green" : "red"}
            isActive={isActive(4)}
            fullWidth
        >
            <div
                style={{
                    ...
                    borderRadius: 8,
                    padding: 12,
                    border: `1px solid ${eq.hasRealSolutions ? "#bbf7d0" : "#fecaca"}`,
                }}
            >
                <div
                    style={{
                        fontWeight: 700,
                        marginBottom: 8,
                        color: eq.hasRealSolutions ? "#166534" : "#991b1b",
                        fontSize: isMobile ? 16 : 18,
                    }}
                >
                    {eq.hasRealSolutions ? "✓ Soluzione" : "✗ Nessuna soluzione reale"}
                </div>
                ...
                {eq.eqType === "completa" && (
                    <div style={{ marginTop: 10, fontSize: 13, color: "#64748b" }}>
                        <Latex>{`\\Delta = ${eq.delta} ${eq.delta < 0 ? "< 0" : eq.delta === 0 ? "= 0" : "> 0"}`}</Latex>
                    </div>
                )}
            </div>
        </StepCard>
    ...
    const MethodContent = (
        <div style={{ fontSize: 13 }}>
            <ol style={{ margin: 0, paddingLeft: 20 }}>
                ...
                <li>
                    <strong>Risoluzione</strong>: metodo adatto (radice / fattorizzazione / Δ)
                </li>
                <li>
                    <strong>Risultato</strong>: scrivi la soluzione in forma insiemistica
                </li>
            </ol>
        </div>
    );
    ...
    if (isMobile) {
        return (
            <DemoContainer title="Equazioni 2° grado" description="Risolvi step-by-step">
    ...
    if (isTablet) {
        return (
            <DemoContainer title="Equazioni di Secondo Grado" description="Risolvi equazioni quadratiche passo dopo passo.">
    ...
    return (
        <DemoContainer title="Equazioni di Secondo Grado" description="Risolvi equazioni quadratiche passo dopo passo.">
            ...
            <InfoBox title="Schema:">{MethodContent}</InfoBox>
        </DemoContainer>
    );
}
