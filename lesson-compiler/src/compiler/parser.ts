 ...
 * Parser - Markdown → AST
 *
 * Converte il Markdown della lezione in un AST intermedio.
 */
...
export class Parser {
  ...
  /**
   * Parse directive body into attributes and content.
   *
   * Rules:
   * - Lines matching `key: value` are attributes (value on same line)
   * - Lines matching `key:` followed by indented/continuation lines are multiline attributes
   * - Lines starting with `-` are content (list items, like quiz options)
   * - Everything else after attributes section is content
   */
  private parseDirectiveBody(body: string): {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmedLine === '') {
        if (currentKey) {
          // End current multiline attribute
          saveCurrentAttr();
        }
        if (inContent) {
          contentLines.push('');
      ...
      // List item (starts with -) → always content
      ...
      // If already in content mode, everything is content
      if (inContent) {
        contentLines.push(line);
        continue;
      }
      ...
      // Continuation of multiline attribute or content
      if (currentKey) {
        currentValue.push(line);
      } else {
        // Not in an attribute, start content
        inContent = true;
        contentLines.push(line);
      }
    }

    // Save any remaining attribute
    saveCurrentAttr();
