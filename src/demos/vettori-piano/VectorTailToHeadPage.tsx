/**
 * VectorTailToHeadDemo - Versione refactorizzata (3 layout)
...
export default function VectorTailToHeadDemo() {
    ...
    // Handlers
    const handleChange =
        (vec: "A" | "B", field: keyof Vector) => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const toolbarWrap: React.CSSProperties = isMobile
        ? { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, width: "100%" }
        : { display: "flex", gap: 8, flexWrap: "wrap" };

    const controlsGridStyle: React.CSSProperties = isMobile
    ...
    const drawHint = drawMode !== "none" ? `Disegna vettore ${drawMode}: clicca la coda, poi la punta` : null;
    ...
    return (
        ...
        >
            {/* Wrapper (coerenza con le altre demo) */}
            <div style={{ maxWidth: 1100, margin: "0 auto" }}>
                ...
                <div style={cardStyle}>
                    ...
                    >
                        ...
                        <div style={toolbarWrap}>
                            ...
                            <button onClick={bringToOrigin} style={btnStyle}>
                                📍 All'origine
                            </button>
                    ...
                    >
                        ...
                        <text x={WIDTH - 15} y={toSvgY(0) - 8} fontSize={14} fill="#374151">
                            x
                        </text>
                        <text x={toSvgX(0) + 8} y={18} fontSize={14} fill="#374151">
                            y
                        </text>
                        ...
                        {[-8, -4, 4, 8].map((v) => (
                            <g key={`tick-${v}`}>
                                ...
                                <text x={toSvgX(0) - 12} y={toSvgY(v) + 4} fontSize={10} textAnchor="end" fill="#6b7280">
                                    {v}
                                </text>
                            </g>
                        ))}
                        ...
                        <text
                            x={toSvgX(vecA.x0 + vecA.dx / 2) + 10}
                            y={toSvgY(vecA.y0 + vecA.dy / 2) - 10}
                            fontSize={14}
                            fill={COLOR_A}
                            fontWeight={600}
                        >
                        ...
                        <text
                            x={toSvgX(vecB.x0 + vecB.dx / 2) + 10}
                            y={toSvgY(vecB.y0 + vecB.dy / 2) - 10}
                            fontSize={14}
                            fill={COLOR_B}
                            fontWeight={600}
                        >
                        ...
                        {showSum && (
                            <>
                                ...
                                <text
                                    x={toSvgX(sumVector.x0 + sumVector.dx / 2) - 20}
                                    y={toSvgY(sumVector.y0 + sumVector.dy / 2) + 20}
                                    fontSize={14}
                                    fill={COLOR_SUM}
                                    fontWeight={600}
                                >
                ...
                <div style={controlsGridStyle}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, gap: 8 }}>
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    padding: "4px 10px",
                                    fontSize: 12,
                                    background: drawMode === "A" ? "#dbeafe" : "#fff",
                                }}
                            >
                                ✏️ Disegna
                            </button>
                  ...
                  <span style={{ color: "#64748b" }}>
                    {field === "x0" ? "x₀" : field === "y0" ? "y₀" : field === "dx" ? "Δx" : "Δy"}
                  </span>
                                    <input
                                        type="number"
                                        step={0.5}
                                        value={vecA[field]}
                                        onChange={handleChange("A", field)}
                                        style={{
                                            width: "100%",
                                            padding: 6,
                                            borderRadius: 6,
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, gap: 8 }}>
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    padding: "4px 10px",
                                    fontSize: 12,
                                    background: drawMode === "B" ? "#dcfce7" : "#fff",
                                }}
                            >
                                ✏️ Disegna
                            </button>
                  ...
                  <span style={{ color: "#64748b" }}>
                    ...
                    <div style={{ ...cardStyle, background: "#fef2f2", borderLeft: `4px solid ${COLOR_SUM}` }}>
                        ...
                        <div style={{ fontSize: 13, display: "grid", gap: 4 }}>
                            <div>
                                <span style={{ color: COLOR_A, fontWeight: 600 }}>A</span> = ({vecA.dx.toFixed(1)}, {vecA.dy.toFixed(1)})
                            </div>
                            <div>
                                <span style={{ color: COLOR_B, fontWeight: 600 }}>B</span> = ({vecB.dx.toFixed(1)}, {vecB.dy.toFixed(1)})
                            </div>
                            <div style={{ borderTop: "1px solid #fecaca", paddingTop: 6, marginTop: 4 }}>
                                <span style={{ color: COLOR_SUM, fontWeight: 700 }}>A + B</span> = ({(vecA.dx + vecB.dx).toFixed(1)},{" "}
                                {(vecA.dy + vecB.dy).toFixed(1)})
                            </div>
                            {showSum && (
                ...
                <div style={{ marginTop: 12, background: "#eff6ff", borderRadius: 12, padding: 12, fontSize: 13, color: "#1e3a8a" }}>
                    <strong>Metodo punta-coda:</strong> Per sommare due vettori, si trasla il secondo vettore in modo che la sua coda coincida con la
                    punta del primo. Il vettore somma va dalla coda del primo alla punta del secondo. Clicca "▶ Punta-coda" per vedere l'animazione.
                </div>
            </div>
        </DemoContainer>
    );
