/**
 * RappresentazioneSoluzioniDisequazioniDemo - Versione refactorizzata con helpers
 */

import React, { useState, useMemo, useRef } from "react";

// Componenti UI
import { DemoContainer, GenerateButton, InfoBox } from "../../components/ui";
...
function algebraicForm(sol: SolutionForm): string {
    switch (sol.type) {
        ...
        case "twoRays": {
            const leftOp = sol.includeLeft ? "≤" : "<";
            const rightOp = sol.includeRight ? "≥" : ">";
            return `x ${leftOp} ${formatNumber(sol.left)}  oppure  x ${rightOp} ${formatNumber(sol.right)}`;
        }
    }
}
        ...
        >
            ...
            <div style={{ border: "1px solid #ddd", borderRadius: 12, padding: 12, marginBottom: 20, boxShadow: "0 2px 6px rgba(0,0,0,0.06)" }}>
                ...
                <svg width={SVG_WIDTH} height={SVG_HEIGHT} style={{ maxWidth: "100%", display: "block", margin: "0 auto" }}>
                    ...
                    <rect x={0} y={0} width={SVG_WIDTH} height={SVG_HEIGHT} fill="url(#graphPaper)" stroke="#ddd" />
                    ...
                    {Array.from({ length: X_MAX - X_MIN + 1 }, (_, i) => {
                        ...
                        return (
                            <g key={`tick-${xReal}`} opacity={tickOpacity}>
                                ...
                                {xReal % 2 === 0 && <text x={xSvg} y={axisY + 22} fontSize={11} textAnchor="middle" fill="#333">{xReal}</text>}
                            </g>
                        );
                    })}
                    ...
                    {segments.map((seg, idx) => {
                        ...
                        return (
                            <g key={`seg-${idx}`}>
                                ...
                                {!seg.isInfiniteLeft && (
                                    <circle cx={xFrom} cy={axisY} r={6} fill={seg.includeFrom ? "#1f77b4" : "#ffffff"} stroke="#1f77b4" strokeWidth={2.5} />
                                )}
                                {!seg.isInfiniteRight && (
                                    <circle cx={xTo} cy={axisY} r={6} fill={seg.includeTo ? "#1f77b4" : "#ffffff"} stroke="#1f77b4" strokeWidth={2.5} />
                                )}
                            </g>
                        );
                    })}
                    ...
                    {showEndpointLabels && endpointValues.map((val, idx) => (
                        <text key={`lbl-${idx}`} x={scaleX(val)} y={axisY - 20} fontSize={13} textAnchor="middle" fill="#1f77b4" fontWeight={600}>
                            {formatNumber(val)}
                        </text>
                    ))}
                </svg>
            ...
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                {showAlgebraic && (
                    <div style={{ flex: 1, minWidth: 250, background: "#eff6ff", borderRadius: 12, padding: 16 }}>
                        <div style={{ fontWeight: 600, marginBottom: 8, color: "#1d4ed8" }}>Forma algebrica</div>
                        <div style={{ fontSize: 20, fontFamily: "Georgia, serif", color: "#1e3a8a" }}>{algebraic}</div>
                    </div>
                )}
                {showInterval && (
                    <div style={{ flex: 1, minWidth: 250, background: "#f0fdf4", borderRadius: 12, padding: 16 }}>
                        <div style={{ fontWeight: 600, marginBottom: 8, color: "#166534" }}>Notazione per intervalli</div>
                        <div style={{ fontSize: 20, fontFamily: "Georgia, serif", color: "#14532d" }}>{interval}</div>
                    </div>
                )}
            </div>
            ...
            <div style={{ marginTop: 20, background: "#fff", borderRadius: 12, padding: 16, border: "1px solid #e5e7eb" }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 20, fontSize: 13 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <span>Tratteggio: verso l'infinito</span>
                    </div>
                </div>
            </div>
