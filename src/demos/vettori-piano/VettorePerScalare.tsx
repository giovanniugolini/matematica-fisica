 ...
 * Versione migliorata con slider, animazione e responsive
 */
...
const Arrow = ({
    ...
    const sx0 = toSvgX(start.x), sy0 = toSvgY(start.y);
    const sx1 = toSvgX(end.x), sy1 = toSvgY(end.y);
    ...
    const hx1 = sx1 - headSize * Math.cos(angle - Math.PI / 6);
    const hy1 = sy1 - headSize * Math.sin(angle - Math.PI / 6);
    const hx2 = sx1 - headSize * Math.cos(angle + Math.PI / 6);
    const hy2 = sy1 - headSize * Math.sin(angle + Math.PI / 6);
    ...
    return (
        <g>
            ...
            {label && (
                <text
                    x={sx1 + 12 * Math.cos(angle - Math.PI / 4)}
                    y={sy1 + 12 * Math.sin(angle - Math.PI / 4)}
                    fontSize={14}
                    fill={color}
                    fontWeight={700}
                    textAnchor="middle"
...
export default function ScalarMultiplicationDemo() {
    ...
    const scaledVector = scale(vector, displayScalar);
    ...
    const gridRange = isMobile ? 5 : 7;
    const ticks = Array.from({ length: gridRange * 2 + 1 }, (_, i) => i - gridRange).filter(v => v !== 0);
        ...
        >
            ...
            <ResponsiveCard style={{ marginTop: 12 }}>
                <ResponsiveSvg width={svgW} height={svgH} maxHeight={{ mobile: "50vh", tablet: "55vh", desktop: "60vh" }}>
                    ...
                    {/* Labels assi */}
                    <text x={svgW - 15} y={toSvgY(0) - 8} fontSize={14} fill={COLORS.axis} fontWeight={600}>x</text>
                    <text x={toSvgX(0) + 8} y={18} fontSize={14} fill={COLORS.axis} fontWeight={600}>y</text>

                    {/* Ticks numerici */}
                    {ticks.filter(v => v % 2 === 0).map(v => (
                        <g key={`tick-${v}`}>
                            <text x={toSvgX(v)} y={toSvgY(0) + 16} fontSize={10} textAnchor="middle" fill="#6b7280">{v}</text>
                            <text x={toSvgX(0) - 8} y={toSvgY(v) + 4} fontSize={10} textAnchor="end" fill="#6b7280">{v}</text>
                        </g>
                    ))}
                ...
                <div style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        ...
                        <span><strong>v</strong> = ({vector.x.toFixed(1)}, {vector.y.toFixed(1)})</span>
                    </div>
                    {showResult && (
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            <div style={{ width: 16, height: 4, background: COLORS.scaled, borderRadius: 2 }} />
                            <span><strong>k·v</strong> = ({scaledVector.x.toFixed(1)}, {scaledVector.y.toFixed(1)})</span>
                        </div>
                    )}
                </div>
            </ResponsiveCard>
                ...
                <ResponsiveCard style={{ marginTop: 12, background: "#f0fdf4" }}>
                    ...
                    <div style={{
                        ...
                        <div>
                            ...
                            <div><strong>k · v</strong> = {scalar} · ({vector.x}, {vector.y})</div>
                        </div>
                        <div>
                            <div style={{ color: "#6b7280", fontSize: 12 }}>Risultato</div>
                            <div><strong>k · v</strong> = ({scaledVector.x.toFixed(2)}, {scaledVector.y.toFixed(2)})</div>
                        </div>
                        <div>
                            <div style={{ color: "#6b7280", fontSize: 12 }}>Moduli</div>
                            <div>|v| = {magnitude(vector).toFixed(2)} → |k·v| = {magnitude(scaledVector).toFixed(2)}</div>
                        </div>
                    </div>
                </ResponsiveCard>
            )}
            ...
            <div style={{
                ...
                <ul style={{ paddingLeft: 20, margin: "8px 0 0 0" }}>
                    <li><strong>k · v = (k·vₓ, k·vᵧ)</strong> – moltiplica ogni componente</li>
                    <li><strong>|k·v| = |k| · |v|</strong> – il modulo viene scalato</li>
                    <li><strong>k &gt; 0</strong>: stessa direzione di v</li>
                    <li><strong>k &lt; 0</strong>: direzione opposta (tratteggiato)</li>
                </ul>
            </div>
        </DemoContainer>
    );
