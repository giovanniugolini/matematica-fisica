...
export default function LimiteInfinitoPiuMenoInfinito() {
    ...
    const getLimitColor = (limit: string) => limit === "+∞" ? "#10b981" : limit === "-∞" ? "#ef4444" : "#f59e0b";
            ...
            <div style={cardStyle}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 8 }}>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                        <select value={direction} onChange={(e) => setDirection(e.target.value as "plus" | "minus")} style={{ padding: "6px 10px", borderRadius: 8, border: "1px solid #cbd5e1" }}>
                            <option value="plus">Verso +∞</option>
                            <option value="minus">Verso -∞</option>
                        </select>
                        <ControlButton onClick={() => setAnimating(true)} disabled={animating}>{animating ? "..." : "▶ Anima"}</ControlButton>
                        <ControlButton onClick={() => { setManualMode(!manualMode); if (!manualMode) setManualX(5); }} active={manualMode}>Manuale</ControlButton>
                ...
                <svg viewBox={`0 0 ${SVG_WIDTH} ${SVG_HEIGHT}`} style={{ width: "100%", height: "auto", maxHeight: "60vh" }}>
                    ...
                    <text x={SVG_WIDTH - 50} y={30} fontSize={16} fill={getLimitColor(selectedFunc.limitPlus)} fontWeight={700}>→ {selectedFunc.limitPlus}</text>
                    <text x={30} y={30} fontSize={16} fill={getLimitColor(selectedFunc.limitMinus)} fontWeight={700}>← {selectedFunc.limitMinus}</text>
                </svg>
                ...
                {manualMode && (
                    <div style={{ marginTop: 12, padding: 12, background: "#f0f9ff", borderRadius: 8 }}>
                        <label style={{ fontSize: 13 }}>
                            ...
                            <input type="range" min={xMin} max={xMax} step={0.2} value={manualX ?? 0} onChange={(e) => setManualX(parseFloat(e.target.value))} style={{ width: "100%", marginTop: 4 }} />
                        </label>
                    </div>
                )}
            </div>
                ...
                <ResultBox title="Limiti all'infinito">
                    <div style={{ display: "grid", gap: 8 }}>
                        <div style={{ padding: 12, background: "#f0fdf4", borderRadius: 8, border: `2px solid ${getLimitColor(selectedFunc.limitPlus)}` }}>
                            ...
                            <div style={{ fontSize: 24, fontWeight: 700, color: getLimitColor(selectedFunc.limitPlus) }}>lim = {selectedFunc.limitPlus}</div>
                        </div>
                        <div style={{ padding: 12, background: "#fffbeb", borderRadius: 8, border: `2px solid ${getLimitColor(selectedFunc.limitMinus)}` }}>
                            ...
                            <div style={{ fontSize: 24, fontWeight: 700, color: getLimitColor(selectedFunc.limitMinus) }}>lim = {selectedFunc.limitMinus}</div>
                        </div>
                    </div>
                    {activeX !== null && activeY !== null && (
                        <div style={{ marginTop: 12, padding: 8, background: "#f0f4f8", borderRadius: 8, fontSize: 12 }}>
                            <div>x = {activeX.toFixed(2)}, f(x) = {Number.isFinite(activeY) ? activeY.toFixed(2) : "grande"}</div>
                        </div>
                    )}
                </ResultBox>
                ...
                <div style={cardStyle}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 11 }}>
                        <div>
                            ...
                            {approachPlus.map((p, i) => <div key={i} style={{ display: "flex", justifyContent: "space-between" }}><span>{p.x.toFixed(1)}</span><span>{p.fx.toFixed(2)}</span></div>)}
                        </div>
                        <div>
                            <div style={{ fontWeight: 600, color: "#f59e0b", marginBottom: 4 }}>x → -∞</div>
                            {approachMinus.map((p, i) => <div key={i} style={{ display: "flex", justifyContent: "space-between" }}><span>{p.x.toFixed(1)}</span><span>{p.fx.toFixed(2)}</span></div>)}
                        </div>
                    </div>
                </div>
            </div>
